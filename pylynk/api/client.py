...
class LynkAPIClient:
    ...
    def _make_request(self, query, variables=None, operation_name=None):
        ...
        try:
            ...
            elapsed_time = time.time() - start_time
            logging.error("RequestException after %.3f seconds: %s", elapsed_time, ex)
    ...
    def download_sbom(self, env_id=None, ver_id=None, env_name=None, 
        ...
        response_data = self._make_request(SBOM_DOWNLOAD_NEW, variables, operation_name="downloadSbom")
        ...
        sbom = response_data.get('data', {}).get('sbom', {}).get('download', {})
        ...
        compression_ratio = (1 - decoded_size / len(b64data)) * 100 if b64data else 0
        logging.debug('Download completed: base64_size=%s, decoded_size=%s, compression=%.2f%%, content_type=%s', 
                     self._format_size(len(b64data)), self._format_size(decoded_size), 
            ...
            try:
                with open(sbom_file, 'rb') as sbom:
                    ...
                    if response.status_code == 401:
                        print("Authentication failed. Please check your INTERLYNK_SECURITY_TOKEN")
    ...
    def print_api_summary(self):
        ...
        avg_time = self._api_call_stats['total_time'] / self._api_call_stats['total_calls']
        avg_size = self._api_call_stats['total_bytes'] / self._api_call_stats['total_calls']
